...
describe("loadStripeMap", () => {
	...
	it("rejects a vat.rate given in percent (24 instead of 0.24)", () => {
		const bad = { ...VALID, vat: { rate: 24, code: "tax-guid" } };
		expect(() => loadStripeMap(writeMap(bad))).toThrow(/fraction/);
	});

	it("treats vatPayable as optional (informational — Merit posts VAT implicitly)", () => {
		const { vatPayable: _vp, ...rest } = VALID.accounts;
		const map = loadStripeMap(writeMap({ ...VALID, accounts: rest }));
		expect(map.accounts.vatPayable).toBeUndefined();
		expect(map.accounts.clearing).toBe("1080");
	});
