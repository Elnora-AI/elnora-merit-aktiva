...
describe("StripeClient.listAll", () => {
	...
	it("throws (never silently truncates) when the page cap is exceeded", async () => {
		// A truncated balance-transaction list would surface only as a confusing imbalance.
		const fetchMock = vi
			.fn()
			.mockImplementation(() =>
				Promise.resolve(jsonResponse(200, { object: "list", data: [{ id: "x" }], has_more: true })),
			);
		vi.stubGlobal("fetch", fetchMock);
		await expect(makeClient().listAll("payouts")).rejects.toMatchObject({ name: "StripeError" });
