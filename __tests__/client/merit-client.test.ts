...
describe("MeritClient.call", () => {
	...
	it("retries a transient 5xx then succeeds", async () => {
		const fetchMock = vi
			...
			.mockResolvedValueOnce(jsonResponse(200, '{"ok":1}'));
		vi.stubGlobal("fetch", fetchMock);
		await expect(makeClient().call("getinvoices")).resolves.toEqual({ ok: 1 });
		expect(fetchMock).toHaveBeenCalledTimes(2);
	});

	it("surfaces an ApiError after a 5xx exhausts its retries", async () => {
		vi.stubGlobal("fetch", vi.fn().mockResolvedValue(jsonResponse(502, "Bad Gateway")));
		await expect(makeClient().call("getinvoices")).rejects.toMatchObject({ name: "ApiError", status: 502 });
	});
