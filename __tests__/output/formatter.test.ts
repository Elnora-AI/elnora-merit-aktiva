...
describe("redactSecrets", () => {
	...
	it("redacts a bare Stripe key token in any position", () => {
		expect(redactSecrets("the key sk_live_abcdefgh123 leaked")).toBe("the key [REDACTED] leaked");
		expect(redactSecrets("rk_test_restricted00 and whsec_secretsecret")).toBe("[REDACTED] and [REDACTED]");
	});
