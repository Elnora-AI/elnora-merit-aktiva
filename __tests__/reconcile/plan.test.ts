...
describe("planPayout", () => {
	...
	it("refuses to book a payout whose currency differs from the map", () => {
		// A USD payout booked with map.currency=EUR would post raw USD minor units as EUR
		// amounts with an EE VAT split applied.
		const plan = planPayout(
			batch({
				payout: { id: "po_usd", amount: 41000, currency: "usd", created: 1, arrival_date: 2, status: "paid" },
				currency: "usd",
			}),
			MAP,
		);
		expect(plan.bookable).toBe(false);
		expect(plan.warnings.some((w) => w.includes("USD") && w.includes("EUR"))).toBe(true);
	});

	it("warns on unclassified deductions (disputes/adjustments folded into other fees)", () => {
		// 1000 minor units of dispute → otherFees; payout shrinks by the same amount so it balances.
		const plan = planPayout(
			batch({
				payout: { id: "po_disp", amount: 40000, currency: "eur", created: 1, arrival_date: 2, status: "paid" },
				otherFeesMinor: 1000,
			}),
			MAP,
		);
		expect(plan.bookable).toBe(true);
		expect(plan.warnings.some((w) => w.includes("unclassified deductions"))).toBe(true);
		expect(plan.fees.stripeFeeMinor).toBe(2500); // folded into the fee bucket
	});
