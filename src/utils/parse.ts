...
/**
 * Read a RAW (non-JSON) request body from `--file <path>` (preferred) or
 * `--data <string>`. For endpoints that take verbatim XML (e-invoice import,
 * camt.053 statement import) — unlike readJsonBody, the content is returned
 * as-is, never parsed. Throws ValidationError if neither flag is set or the
 * file is unreadable. `what` names the expected content in error messages.
 */
export function readRawBody(opts: { data?: string; file?: string }, what = "request body"): string {
	if (opts.file) {
		try {
	...
	throw new ValidationError(`No ${what} provided.`, `Pass --file <path> or --data '<content>'.`);
}
