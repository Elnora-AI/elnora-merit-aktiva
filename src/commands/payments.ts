...
export function setupPaymentsCommand(program: Command): void {
	...
	grp
		...
		.description(
			"Register a payment against a purchase invoice (vendor bill). Endpoint: POST /api/v2/sendPaymentV. PaymentDate format YYYYmmddHHii. Use v2 when not in local currency (CurrencyCode required; omit CurrencyRate for the ECB rate).",
		)
		.option("--bank-id <guid>", "Bank / payment-method id")
	...
	grp
		...
		.description(
			"Register a (pre)payment against a sales offer. Endpoint: POST /api/v2/sendPaymentO. PaymentDate format YYYYmmddHHii. Use v2 when not in local currency (CurrencyCode required; omit CurrencyRate for the ECB rate).",
		)
		.option("--bank-id <guid>", "Bank / payment-method id")
		.option("--iban <iban>", "IBAN")
		.option("--customer-name <name>", "Customer name")
...
/**
 * Read a raw XML body from --file (preferred) or --data. The statement-import
 * endpoint takes verbatim camt.053 XML, so this returns the raw string rather
 * than parsing JSON like readJsonBody. Throws ValidationError if neither is set.
 */
function readXmlBody(opts: { data?: string; file?: string }): string {
	if (opts.file) {
		try {
			return readFileSync(opts.file, "utf8");
		} catch (err) {
			throw new ValidationError(`Could not read --file ${opts.file}: ${(err as Error).message}`);
		}
	}
	if (opts.data) return opts.data;
	throw new ValidationError("No camt.053 XML provided.", "Pass --file <path.xml> or --data '<xml>'.");
}
