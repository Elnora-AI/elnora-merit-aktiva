...
// ONE summary GL batch: card sales recognised GROSS as revenue + 24% output VAT, fees
// as an expense, all through a clearing account. The real LHV bank-import row then
...
export function setupReconcileCommand(program: Command): void {
	...
	grp
		.command("init")
		.description(
			"Write a placeholder stripe-map.json and print candidate values (chart of accounts, banks, VAT codes) from Merit so you can fill it in. Does not overwrite an existing map without --force.",
		)
	...
	grp
		...
		.action(
			handleAsyncCommand(async (opts: { map?: string; payout?: string; since?: string }) => {
			...
			handleAsyncCommand(
				async (opts: {
					...
					for (const payout of payouts) {
						...
						const plan = planPayout(batch, map);
						if (!plan.bookable) {
							skipped.push({ payoutId: payout.id, reason: "does not balance", warnings: plan.warnings });
							continue;
						}
