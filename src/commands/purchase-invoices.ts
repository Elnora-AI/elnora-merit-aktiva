...
export function setupPurchaseInvoicesCommand(program: Command): void {
				...
				async (opts: {
					...
					const body = resolveBody(
						readJsonBody(opts),
						{
							PeriodStart: parseDateYmd(opts.periodStart, "--period-start"),
							PeriodEnd: parseDateYmd(opts.periodEnd, "--period-end"),
							DateFilter: parsePositiveInt(opts.dateFilter, "--date-filter"),
							StatusFilter: parsePositiveInt(opts.statusFilter, "--status-filter"),
	...
	grp
		.command("pay")
		.description(
			"Create a payment of a purchase invoice. Endpoint: POST /api/v1/sendPaymentV. Use --v2 when the payment is NOT in local currency (v2 adds CurrencyCode and CurrencyRate; if CurrencyRate is omitted Merit uses the EU central bank rate for the date). PaymentDate format is YYYYmmddHHii; Amount is decimal.",
		)
		...
		.option("--payment-date <date>", "Payment date (YYYYmmddHHii; YYYY-MM-DD or YYYYMMDD also accepted)")
				...
				async (opts: {
					...
					const body = resolveBody(
						readJsonBody(opts),
						{
							...
							CurrencyRate: opts.currencyRate,
						},
						{ required: false },
					);
