...
import { parseBool, parseDateYmd, readJsonBody, readRawBody, requireYes, resolveBody } from "../utils/index.js";
...
export function setupSalesInvoicesCommand(program: Command): void {
	...
	grp
		...
		.option("--data <xml>", "Raw e-invoice XML request body")
		.option("--file <path>", "Path to a file with the raw XML request body")
		.action(
			handleAsyncCommand(async (opts: { data?: string; file?: string }) => {
				const client = await getClient();
				// The body is verbatim e-invoice XML — it must NOT go through readJsonBody.
				const body = readRawBody(opts, "e-invoice XML");
