...
// booked as ONE summary GL batch — gross card sales recognised as revenue +
// output VAT, Stripe fees as an expense, all through a clearing account that the
// real bank-import row then clears in the Merit UI. Card sales are aggregated
...
export function planPayout(batch: PayoutBatch, map: StripeMap): PayoutPlan {
	...
	// One map = one currency: the GL batch is stamped with map.currency and the VAT
	// split assumes that jurisdiction, so a payout in any other currency would book
	// raw foreign minor units as if they were map-currency amounts. Refuse.
	const currencyMismatch = batch.currency.toUpperCase() !== map.currency.toUpperCase();
	if (currencyMismatch) {
		warnings.push(
			`Payout ${batch.payout.id} is in ${batch.currency.toUpperCase()} but the map is for ${map.currency.toUpperCase()} — refusing to book. Use a separate map (and clearing/revenue accounts) per currency.`,
		);
	}
	...
	if (batch.otherFeesMinor > 0) {
		warnings.push(
			`Payout ${batch.payout.id} contains ${formatMinor(batch.otherFeesMinor)} in unclassified deductions (disputes, adjustments, or other non-fee transactions) — these would be booked as Stripe processing fees with NO output-VAT adjustment. Review before booking; a chargeback usually needs its own contra entry and a VAT correction.`,
		);
	}
	...
	return {
		...
		imbalanceMinor,
		warnings,
		bookable: imbalanceMinor === 0 && !period.blocking && !currencyMismatch,
	};
}
