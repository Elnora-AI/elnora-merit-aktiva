...
export interface ChargeItem {
	...
	description: string | null;
	// VAT from Stripe Tax, when available. Currently always null — the Charge object
	// carries no tax amount; authoritative Stripe Tax needs the Tax Transactions API
	// (future work). Kept so splitVat's mismatch detection works once it is wired up.
	stripeTaxMinor: number | null;
...
export interface StripeMap {
	...
	accounts: {
		revenue: string;
		// Informational only: Merit posts the output VAT implicitly from the revenue
		// line's TaxId — the connector never writes to this account itself (an explicit
		// VAT row would double-post). Optional.
		vatPayable?: string;
		stripeFees: string;
		platformFees: string;
		refunds: string;
		// GL code of the Stripe clearing account. The GL batch debits the gross into
		// this account, the fee GL credits it, and the real bank-import row clears
