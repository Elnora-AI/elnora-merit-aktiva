...
// The clearing account is left holding exactly the payout NET, which the real
...
export function buildPayoutGl(plan: PayoutPlan, map: StripeMap): Record<string, unknown> | null {
	...
	// Every row is emitted only when non-zero: a fees-only or refunds-only payout must
	// not carry a 0.00 sales row (Merit may reject zero lines, and a VAT-tagged zero
	// row would put an empty entry on the KMD).
	const rows: GlRow[] = [];
	if (plan.grossMinor > 0) {
		rows.push(
			{
...
export async function executePlan(client: MeritClient, plan: PayoutPlan, map: StripeMap): Promise<ExecuteResult> {
	...
	// Merit signals some business rejections as 200 + plain text (e.g. an unbalanced-
	// entry message in Estonian). The client returns that raw string; treating it as
	// success would ledger the payout as booked while nothing posted. Verified success
	// responses are JSON or an empty body — fail on any textual response so the payout
	// stays out of the ledger and is retried after the cause is fixed.
	if (typeof glResult === "string") {
		throw new ApiError(200, `Merit rejected the GL batch for payout ${plan.payoutId}: ${glResult.slice(0, 500)}`, {
			endpoint: "sendglbatch",
			body: glResult.slice(0, 2000),
		});
	}
