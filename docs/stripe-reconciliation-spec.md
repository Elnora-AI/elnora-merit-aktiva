...
The original framing was Luma → Stripe → Merit. The money flow is simpler than that:
...
payout arrival date), using the GL codes from the map:

```
Dr  accounts.clearing      gross           card sales, VAT-inclusive
    Cr  accounts.revenue       net (gross − VAT)   tagged with the VAT TaxId (KMD)
    Cr  accounts.vatPayable    vat                 24% output VAT
Dr  accounts.stripeFees    stripeFee       Stripe processing fees (expense)
Dr  accounts.platformFees  platformFee     Luma 5% / other application fees (expense)
...
The batch **balances by construction**: Σ debit == Σ credit (gross = net + VAT, and
every fee/refund debit is offset by a matching clearing credit). The connector
asserts the Stripe-side identity
...
The output VAT is posted explicitly to `accounts.vatPayable`. In addition, the
**revenue line carries the VAT `TaxId`** (`vat.code`) and the VAT amount as
declaration metadata, so Merit's KMD (VAT return) **auto-populates** from the base
line. The `TaxId`/`VatAmount` on the revenue row is declaration metadata only — the
explicit VAT-account line is what does the posting, so there is no double-posting.
...
populated by the user (the public-repo placeholder/sync pattern). No GL codes, VAT
rates, or account IDs in the source.
...
```jsonc
{
  ...
  "accounts": {
    ...
    "vatPayable": "<gl code>",       // output VAT liability
...
- **Configured rate** (`vat.rate`): apply the configured rate to ticket revenue.
  Correct for single-rate channels — e.g. single-rate EE event sales are uniformly 24%.
- **Stripe Tax**: when the charge carries a Stripe Tax line, that VAT figure is
  authoritative and the split uses it instead of the configured rate. If the Stripe
  Tax line disagrees with the configured rate, the connector keeps the Stripe figure
  and emits a per-charge warning in the preview.
...
For a publishable tool, depend on the official `stripe` npm SDK (read-only secret key),
not on any internal stripe-cli plugin. Config:
...
`reconcile run --yes --since <last>`, matching the existing finance-pipeline pattern.
