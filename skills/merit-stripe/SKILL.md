...
2. **Stripe processing fees are a VAT-EXEMPT payment service — NO reverse charge.** Book
   them to a bank-service expense (4340) like an ordinary pangateenus, with no `TaxId` and
...
- **Don't create a Merit sales invoice for a Stripe invoice that was paid by card.** A
  ...
  in a payout = card (→ payout, no invoice); no charge plus a real LHV deposit referencing the
