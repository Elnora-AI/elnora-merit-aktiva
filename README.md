...
elnora-merit reports income-statement --data '{"PeriodStart":"20260101","PeriodEnd":"20260331"}'
...
- **Exit codes:** `0` success · `1` general · `2` validation · `3` auth · `4` not found · `5` rate limited · `6` API error.
...
24% for EE, with the VAT TaxId tagged on the revenue line so Merit's KMD auto-populates) and
**output VAT**; Stripe and platform fees are debited as a separate expense and credited back
to clearing (revenue is booked GROSS with fees as a cost — ASC 606 / IFRS 15 principal
treatment). The clearing account is left holding exactly the payout net, which your real
